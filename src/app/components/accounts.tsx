...
const defaultTasks = [
    {name:"Dishes", amount: 1, parentView: true, state: TaskState.Pending, info: "Do that dishes when they pile up.", actions: [TaskAction.Info, TaskAction.Approve], completedOn: new Date("04/09/2024"), external: false},
    ...
    {name:"Mow", amount: 20, parentView: true, state: TaskState.Available, info: "Mow the lawn for 80-year old neighbor, Fred. Bring your own lawn mower and knock at the door beforehand.", actions: [TaskAction.Info], completeBy: new Date("04/21/2024"), external: true},
    {name:"Wash Car", amount: 10, parentView: true, state: TaskState.Available, info: "Wash the car for Johny. Soap, water and sponges will be provided. Tasks will be done on the complete by date", actions: [TaskAction.Info], completeBy: new Date("04/21/2024"), external: true},
] as Task[];
...
export function saveUser(user?: User) {
    if (user) {
        window.sessionStorage.setItem("famdit-user", JSON.stringify(user, (key, value) => key === "parent" ? undefined : value));
    }
}
